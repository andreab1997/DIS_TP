...
def Cq_3_til_reg(z,Q,p):
    q = [p[0],Q] #in the place of Q then there will be the factorization scale
    return Cq_3_m_reg(z,Q,p) + Cq_2_m_reg(z,Q,p)*Mgg_1_loc(z,q) - Convolute(Cg_1_m_reg,Mgq_2_reg,z,Q,p) - 2*(Cb_1_loc(z,Q)*Mbq_2(z,q) + Convolute(Cb_1_reg,Mbq_2,z,Q,p) + Convolute_plus_coeff(Cb_1_sing,Mbq_2,z,Q,p)) -2*(Cb_0_loc(z,Q)*Mbq_2(z,q))
#FL
...
def CLg_3_til_reg(z,Q,p):
    q = [p[0],Q] #in the place of Q then there will be the factorization scale
    return CLg_3_m_reg(z,Q,p) + P2(q)*CLg_1_m_reg(z,Q,p) - (CLg_1_m_reg(z,Q,p)*Mgg_2_loc(z,q) + Convolute(CLg_1_m_reg,Mgg_2_reg,z,Q,p) + Convolute_plus_matching(CLg_1_m_reg,Mgg_2_sing,z,Q,p)) -2*(Convolute(CLb_1_reg,Mbg_2,z,Q,p) - CLb1_Mbg1(z)*Mgg_1_loc(z,q)) -2*(CLb_2_loc(z,Q)*Mbg_1(z,q) + Convolute(CLb_2_reg,Mbg_1,z,Q,p))
...
def CLq_3_til_reg(z,Q,p):
    q = [p[0],Q] #in the place of Q then there will be the factorization scale
    return CLq_3_m_reg(z,Q,p) + CLq_2_m_reg(z,Q,p)*Mgg_1_loc(z,q) - Convolute(CLg_1_m_reg,Mgq_2_reg,z,Q,p) - 2*Convolute(CLb_1_reg,Mbq_2,z,Q,p) 
