...
def test_Talbot_parameters():
    ...
    for x in np.geomspace(1e-4, 1, 10, endpoint=False):
        tmp1 = Mbq_3_reg_inv(x, [Q, mass], nf, r=2, s=0)
        tmp2 = Mbq_3_reg_inv(x, [Q, mass], nf)
        assert_allclose(tmp1, tmp2, rtol=1e-6)

    for x in np.geomspace(1e-4, 1, 10, endpoint=False):
        tmp1 = Mbg_3_reg_inv(x, [Q, mass], nf, r=2, s=0)
        tmp2 = Mbg_3_reg_inv(x, [Q, mass], nf)
        assert_allclose(tmp1, tmp2, rtol=1e-6)
...
def test_Talbot_linear_path():
    ...
    for x in np.geomspace(1e-4, 1.0, 10, endpoint=False):
        tmp1 = Mbq_3_reg_inv(x, [Q, mass], nf)
        tmp2 = Mbq_3_reg_inv(x, [Q, mass], nf, path="linear")
        assert_allclose(tmp1, tmp2, rtol=1e-6)

    for x in np.geomspace(1e-4, 1.0, 10, endpoint=False):
        tmp1 = Mbg_3_reg_inv(x, [Q, mass], nf)
        tmp2 = Mbg_3_reg_inv(x, [Q, mass], nf, path="linear")
        assert_allclose(tmp1, tmp2, rtol=1e-6)
...
def test_linear_parameters():
    ...
    for x in np.geomspace(1e-4, 1.0, 10, endpoint=False):
        tmp1 = Mbq_3_reg_inv(x, [Q, mass], nf, path="linear")
        tmp2 = Mbq_3_reg_inv(x, [Q, mass], nf, r=2.0, s=2.5, path="linear")
        assert_allclose(tmp1, tmp2, rtol=1e-6)

    for x in np.geomspace(1e-4, 1.0, 10, endpoint=False):
        tmp1 = Mbg_3_reg_inv(x, [Q, mass], nf, path="linear")
        tmp2 = Mbg_3_reg_inv(x, [Q, mass], nf, r=2.0, s=2.5, path="linear")
        assert_allclose(tmp1, tmp2, rtol=1e-6)
