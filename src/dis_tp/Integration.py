...
from .parameters import pids, number_active_flavors
...
def F2_FO(order, pdf, x, Q, h_id, muF_ratio=1, muR_ratio=1):
    ...
    if order >= 1:
        res += (
            ...
            * Mypdf.alphasQ(muR)
            * PDFConvolute(
                MassiveCoeffFunc.Cg_1_m_reg,
                Mypdf,
                x,
                Q,
                g_id,
                p1=[Mypdf.quarkMass(h_id)],
            )
        )
    if order >= 2:
        res += pow((1 / (4 * np.pi)) * Mypdf.alphasQ(muR), 2) * (
            PDFConvolute(
                MassiveCoeffFunc.Cg_2_m_reg,
                Mypdf,
                x,
                Q,
                g_id,
...
def F2_R(order, pdf, x, Q, h_id, muF_ratio=1, muR_ratio=1):
    ...
    if order >= 1:
        ...
        nll_local = MasslessCoeffFunc.Cb_0_loc(x, Q) * (
    ...
    if order >= 2:
        ...
        nnll_local = (
            (1 / (4 * np.pi))
            * Mypdf.alphasQ(muR)
            * MasslessCoeffFunc.Cb_1_loc(x, Q)
            * (Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q))
        )
        nnll_sing = (
            (1 / (4 * np.pi))
    ...
    if order >= 3:
        ...
        n3ll_local = (((1 / (4 * np.pi)) * Mypdf.alphasQ(muR)) ** 2) * (
            MasslessCoeffFunc.Cb_2_loc(x, Q, nf)
            * (Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q))
            + (1 / (4 * np.pi))
            * Mypdf.alphasQ(muR)
            * (
                MasslessCoeffFunc.Cg_3_loc(x, Q, nf) * Mypdf.xfxQ2(h_id, x, Q * Q)
                + MasslessCoeffFunc.Cq_3_loc(x, Q, nf)
                * (
                    Mypdf.xfxQ2(1, x, Q * Q)
                    + Mypdf.xfxQ2(-1, x, Q * Q)
                    + Mypdf.xfxQ2(2, x, Q * Q)
...
def F2_M(order, meth, pdf, x, Q, h_id, muF_ratio=1, muR_ratio=1):
    ...
    if meth == "our":
        ...
        if order >= 1:
            ...
            nlo_nll_local = MasslessCoeffFunc.Cb_0_loc(x, Q) * (
                Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q)
            )
    ...
    if meth == "fonll":
        if order >= 0:
            res += MasslessCoeffFunc.Cb_0_loc(x, Q) * (
                Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q)
            )
        if order >= 1:
            nlo_nll_reg = (
        ...
        if order >= 3:
            ...
            n3lo_n3ll_local = (
                ...
                * (1 / (4 * np.pi))
                * Mypdf.alphasQ(muR)
                * MasslessCoeffFunc.Cb_3_loc(x, Q, nf)
                * (Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q))
            )
            n3lo_n3ll_sing = (
                (1 / (4 * np.pi))
...
def FL_R(order, pdf, x, Q, h_id, muF_ratio=1, muR_ratio=1):
    ...
    if order >= 3:
        ...
        n3ll_loc = (((1 / (4 * np.pi)) * Mypdf.alphasQ(muR)) ** 2) * (
            MasslessCoeffFunc.CLb_2_loc(x, Q)
            * (Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q))
        )
    ...
    if meth == "fonll":
        ...
        if order >= 3:
            ...
            n3lo_n3ll_loc = (
                ...
                * (1 / (4 * np.pi))
                * Mypdf.alphasQ(muR)
                * MasslessCoeffFunc.CLb_3_loc(x, Q, nf)
                * (Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q))
            )
