...
def main():
    ...
    while not main_menu_exit:
        ...
        if main_sel == 0:
            ...
            if len(Stru_func) == 0:
                print(
                    "At least one Structure function is needed. Returning to the main menu..."
                )
                check = False
            elif len(Methods) == 0:
                ...
                print(
                    "At least one perturbative order is needed. Returning to the main menu..."
                )
                check = False
            if check:
                ...
                if "fonll" in Methods and "n3lo" in orders:
                    print(
                        "WARNING: Not all the results are available with FONLL method"
                    )
                main_menu_exit = True
...
def do_the_calculation(Stru_func, Methods, map_PDFsorder, scalevar):
    ...
    numberofcalc = (
        len(Stru_func) * len(map_PDFsorder)
        + len(Stru_func) * len(map_PDFsorder) * len(Methods)
    ) + 1
                ...
                for met in Methods:
                    if Sf == "F2":
                        ...
                        if met == "our":
                            ...
                            resR = np.array(
                                [[[0.0 for q in Q] for x in X] for ratio in ratios]
                            )
                    elif Sf == "FL":
                        resM = np.array(
                            [
                ...
                for x in X:
                    ...
                    for thr in thre:
                        ...
                        with open(
                            ...
                            writer.writerows(
                                zip(Q, M[Sf][order][meth][thre.index(thr)][X.index(x)])
                            )
