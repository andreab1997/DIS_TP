...
def F2_FO(
    ...
    if order >= 1:
        res += a_s * PDFConvolute(
            MassiveCoeffFunc.Cg_1_m_reg, Mypdf, x, Q, p, h_id, g_id, muR_ratio
        )
    if order >= 2:
        res += a_s**2 * (
            PDFConvolute(
                MassiveCoeffFunc.Cg_2_m_reg, Mypdf, x, Q, p, h_id, g_id, muR_ratio
            )
            + conv_func(
        ...
        for ihq in range(h_id + 1, 6):
            ...
            reg_miss = PDFConvolute(
                MassiveCoeffFunc.Cb_2_m_reg, Mypdf, x, Q, pihq, h_id, h_id, muR_ratio
            )
            loc_miss = MassiveCoeffFunc.Cb_2_m_loc(x, Q, pihq, h_id, muR_ratio) * (
    ...
    if order >= 3:
        res += a_s**3 * (
            PDFConvolute(
                MassiveCoeffFunc.Cg_3_m_reg, Mypdf, x, Q, p, h_id, g_id, muR_ratio
            )
            + conv_func(
...
def F2_R(order, pdf, x, Q, h_id, meth=None, target_dict=None, muF_ratio=1, muR_ratio=1):
    ...
    if order >= 1:
        nll_reg = a_s * PDFConvolute(
            MasslessCoeffFunc.Cg_1_reg, Mypdf, x, Q, p, nf, g_id, muR_ratio
        )
        nll_local = MasslessCoeffFunc.Cb_0_loc(x, Q, p, nf, muR_ratio) * (
            Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q)
        )
    ...
    if order >= 2:
        nnll_reg = a_s * (
            ...
            * (
                PDFConvolute(
                    MasslessCoeffFunc.Cg_2_reg, Mypdf, x, Q, p, nf, g_id, muR_ratio
                )
                + PDFConvolute(
            ...
            + PDFConvolute(
                MasslessCoeffFunc.Cb_1_reg, Mypdf, x, Q, p, nf, h_id, muR_ratio
            )
        )
        nnll_local = (
            a_s
            * MasslessCoeffFunc.Cb_1_loc(x, Q, p, nf, muR_ratio)
            * (Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q))
        )
        nnll_sing = a_s * PDFConvolute_plus(
            MasslessCoeffFunc.Cb_1_sing, Mypdf, x, Q, p, nf, h_id, muR_ratio
        )
    ...
    if order >= 3:
        ...
        n3ll_reg = (a_s**2) * (
            ...
            * (
                PDFConvolute(
                    MasslessCoeffFunc.Cg_3_reg, Mypdf, x, Q, pg, nf, g_id, muR_ratio
                )
                + PDFConvolute(
            ...
            + PDFConvolute(
                MasslessCoeffFunc.Cb_2_reg, Mypdf, x, Q, p, nf, h_id, muR_ratio
            )
        )

        n3ll_local = (a_s**2) * (
            MasslessCoeffFunc.Cb_2_loc(x, Q, p, nf, muR_ratio)
            * (Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q))
            + a_s
            * (
                MasslessCoeffFunc.Cg_3_loc(x, Q, pg, nf, muR_ratio)
                * Mypdf.xfxQ2(h_id, x, Q * Q)
                + MasslessCoeffFunc.Cq_3_loc(x, Q, ps, nf, muR_ratio)
                * singlet_pdf(Mypdf, x, Q, nf, target_dict)
            )
        )
        n3ll_sing = a_s**2 * PDFConvolute_plus(
            MasslessCoeffFunc.Cb_2_sing, Mypdf, x, Q, p, nf, h_id, muR_ratio
        )
...
def F2_M(order, pdf, x, Q, h_id, meth, target_dict=None, muF_ratio=1, muR_ratio=1):
    ...
    if meth == "our":
        ...
        if order >= 1:
            nlo_nll_reg = a_s * PDFConvolute(
                TildeCoeffFunc.Cg_1_til_reg, Mypdf, x, Q, p, nf, g_id, muR_ratio
            )
            nlo_nll_local = MasslessCoeffFunc.Cb_0_loc(x, Q, p, nf, muR_ratio) * (
                Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q)
            )
        ...
        if order >= 2:
            nnlo_nnll_reg = a_s * (
                ...
                * (
                    PDFConvolute(
                        TildeCoeffFunc.Cg_2_til_reg, Mypdf, x, Q, p, nf, g_id, muR_ratio
                    )
                    + PDFConvolute(
        ...
        if order >= 3:
            n3lo_n3ll_reg = (a_s**2) * (
                ...
                * (
                    PDFConvolute(
                        TildeCoeffFunc.Cg_3_til_reg, Mypdf, x, Q, p, nf, g_id, muR_ratio
                    )
                    + PDFConvolute(
    ...
    if meth == "fonll":
        if order >= 0:
            res = MasslessCoeffFunc.Cb_0_loc(x, Q, p, nf, muR_ratio) * (
                Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q)
            )
        if order >= 1:
            nlo_nll_reg = a_s * (
        ...
        if order >= 2:
            ...
            for ihq in range(h_id + 1, 6):
                pihq = [masses(ihq), Q, charges(h_id)]
                reg_miss = PDFConvolute(
                    MassiveCoeffFunc.Cb_2_m_reg, Mypdf, x, Q, pihq, nf, h_id, muR_ratio
                )
                loc_miss = MassiveCoeffFunc.Cb_2_m_loc(x, Q, pihq, nf, muR_ratio) * (
                    Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q)
                )
                res += a_s**2 * (reg_miss + loc_miss)
        ...
        if order >= 3:
            ...
            n3lo_n3ll_reg = a_s**3 * (
                ...
                + PDFConvolute(
                    MasslessCoeffFunc.Cb_3_reg, Mypdf, x, Q, pns, nf, h_id, muR_ratio
                )
            )
            n3lo_n3ll_local = (
                a_s**3
                * MasslessCoeffFunc.Cb_3_loc(x, Q, pns, nf, muR_ratio)
                * (Mypdf.xfxQ2(h_id, x, Q * Q) + Mypdf.xfxQ2(-h_id, x, Q * Q))
            )
            n3lo_n3ll_sing = a_s**3 * PDFConvolute_plus(
                MasslessCoeffFunc.Cb_3_sing, Mypdf, x, Q, pns, nf, h_id, muR_ratio
            )
            res += n3lo_n3ll_reg + n3lo_n3ll_local + n3lo_n3ll_sing
    return res
