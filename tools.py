...
def PDFConvolute(func1, pdf, x, Q, pid, p1=None):
    ...
    upper = 1.
    if pid == 21:
        resultmid, errormid = integrate.quad(lambda z: func1(z,Q,p1)*pdf.xfxQ2(pid, x*(1./z),Q*Q),lower,upper, epsrel = 1.e-02,points=(x,1.))
        result = resultmid
        errormid = 0 
        error = errormid
    if pid == 5:
        resultmid, errormid = integrate.quad(lambda z: func1(z,Q,p1)*(pdf.xfxQ2(pid, x*(1./z),Q*Q) + pdf.xfxQ2(-pid, x*(1./z),Q*Q)),lower,upper, epsrel = 1.e-02,points=(x,1.))
        result = resultmid 
        errormid = 0 
        error = errormid
    if pid == 1:
        resultmid, errormid = integrate.quad(lambda z: func1(z,Q,p1)*(pdf.xfxQ2(1, x*(1./z),Q*Q) + pdf.xfxQ2(2, x*(1./z),Q*Q) + pdf.xfxQ2(3, x*(1./z),Q*Q) + pdf.xfxQ2(4, x*(1./z),Q*Q) + pdf.xfxQ2(-1, x*(1./z),Q*Q) + pdf.xfxQ2(-2, x*(1./z),Q*Q) + pdf.xfxQ2(-3, x*(1./z),Q*Q) + pdf.xfxQ2(-4, x*(1./z),Q*Q)),lower,upper, epsrel = 1.e-02,points=(x,1.))
        result = resultmid 
        errormid = 0 
        error = errormid
    return result
...
def Convolute(func1,matching, x,Q,p1=None):
    ...
    upper = 1.
    q = [p1[0],Q]
    resultmid, errormid = integrate.quad(lambda z: (1./z)*func1(z,Q,p1)*matching(x*(1./z),q),lower,upper, epsrel = 1.e-02,points=(x,1.))
    return resultmid
...
def Convolute_matching(matching1,matching2, x,Q,p1=None):
    ...
    upper = 1.
    q = [p1[0],Q]
    resultmid, errormid = integrate.quad(lambda z: (1./z)*matching1(z,q)*matching2(x*(1./z),q),lower,upper, epsrel = 1.e-02,points=(x,1.))
    return resultmid
